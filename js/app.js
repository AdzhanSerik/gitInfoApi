...
function createProfileEl(profileData) {
  ...
  element.innerHTML = `
    ...
    <p class="search-text"><span>Город: </span><a href="${profileData.url}">Посмотреть на гите</a></p>
    <p class="search-text"><span>Подписчики: </span>${profileData.followers}</p>
    <p class="search-text"><span>Подписки: </span>${profileData.following}</p>
    <p class="search-text"><span>Проекты: </span>${profileData.public_repos}</p>
  `
